...
Do not modify generated files, they get over-written. You can safely re-run the preprocessor in the same directory as other existing generated files.
...
The proper state struct and command enum must be created; the generator doesn't do that for you. The first element of the command queue should be a no operation (`NOP`), as an empty queue will return the first item when attempting to dequeue.
