...
#if !defined(_AB_GENERATED_HEADER_GENERATEDSAMPLE_)
#define _AB_GENERATED_HEADER_GENERATEDSAMPLE_
...
constexpr abs_stringptr EnumToString(EnumClass1 EnumToken);
constexpr char const* EnumToCString(EnumClass1 EnumToken);
#ifdef GEN_JSMN_HEADER
...
#endif // _AB_GENERATED_HEADER_GENERATEDSAMPLE_

#if defined(GENERATEDSAMPLE_SRC)
#undef GENERATEDSAMPLE_SRC
...
constexpr abs_stringptr EnumClass1_Strings[EnumClass1_Count] = 
{
   {"EC1_Value1", 10},
   {"EC1_Value2", 10},
   {"EC1_Value3", 10},
...
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S11\":%d,",  (Value.S11));
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S12\":%d,",  (Value.S12));
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S13\":%d,",  (Value.S13));
...
if(Value.S18)
{
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S18\":\"%s\",", (Value.S18));
}
if(Value.S19)
{
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S19\":\"%s\",", (Value.S19));
}
{
size_t StringLen = abs_StringLength(Value.S1A, 10);
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S1A\":\"%.*s\",", (s32)StringLen, Value.S1A);
}
{
size_t StringLen = abs_StringLength(Value.S1B, 10);
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S1B\":\"%.*s\",", (s32)StringLen, Value.S1B);
}
{
size_t StringLen = abs_StringLength(Value.S1C, 10);
Length += stbsp_snprintf(&Json[Length], (MaxLength - Length), "\"S1C\":\"%.*s\" ", (s32)StringLen, Value.S1C);
}
...
jsmntok_t *JsonToObject(memory_arena *VolatileMemory, char const *Json, size_t JsonLength, jsmntok_t *TokenArray, Struct1 *ObjectOut, Struct1_existlist *ItemsExistOut)
{
    Struct1_existlist* ItemExists = abm_PushStruct(VolatileMemory, Struct1_existlist);
    ...
    if(Token)
    {
        ...
        for(s32 Index = 0; Index < NumTokensToProcess; ++Index)
        {
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S11",(ArrayCount("S11")-1), true)) && 
...
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S11,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S12",(ArrayCount("S12")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))) 
{
            ItemExists->S12 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S12,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S13",(ArrayCount("S13")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))) 
{
            ItemExists->S13 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S13,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S14",(ArrayCount("S14")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))) 
{
            ItemExists->S14 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S14,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S15",(ArrayCount("S15")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))) 
{
            ItemExists->S15 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S15,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S16",(ArrayCount("S16")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))) 
{
            ItemExists->S16 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,  &ObjectOut->S16,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S17",(ArrayCount("S17")-1), true)) && 
               (((Token+1)->type == JSMN_STRING) || ((Token+1)->type == JSMN_OBJECT))&& 
(ObjectOut->S17)) 
{
            ItemExists->S17 = true;
Token = JsonToObject(VolatileMemory, Json, JsonLength, ++Token,   ObjectOut->S17,0);
            ...
            if((abs_AreStringsEqual(&Json[Token->start], TokenLength, "S1A",(ArrayCount("S1A")-1), true)) && 
...
JsonArrayToObjectArray(memory_arena *VolatileMemory, char const *Json, size_t JsonLength, Struct1 **ObjectArray, Struct1_existlist **ObjectArrayExists)
{
   u32 NumberOfObjects = 0;
    ...
    if(TokenArray)
    {
        ...
        if (!(*ObjectArray))
        {
            *ObjectArray = abm_PushArray(VolatileMemory, NumberOfObjects, Struct1);
        }
        if (!(*ObjectArrayExists))
        {
            *ObjectArrayExists = abm_PushArray(VolatileMemory, NumberOfObjects, Struct1_existlist);
        }
